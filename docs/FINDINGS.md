...
Lives here, in the private repo, on purpose. The incident repo is public and is Neo's
working directory; nothing in it names these.
...
`./add-db-sg.sh` creates a security group with the raw AWS CLI — standing
...
`./cleanup.sh` purges the queues. It also calls `./remove-db-sg.sh`, which
detaches and deletes the security group, so a rehearsal always starts from the
same place. Re-arm with `./add-db-sg.sh`.
